...
raw_ostream &dumpAccess(raw_ostream &OS, Access Acc,
...
AccNode *InstDF::getUses(const BinaryContext &BC, Access Acc) const {
  return InstAccesses[calculateAccessIndex(BC, Acc)];
}
void InstDF::setDef(const BinaryContext &BC, Access Acc, Access Def) {
  assert(!InstAccesses[calculateAccessIndex(BC, Acc)]);
  InstAccesses[calculateAccessIndex(BC, Acc)] = new AccNode(Def);
}
void InstDF::addUse(const BinaryContext &BC, Access Acc, Access Use) {
...
bool InstDF::removeDef(const BinaryContext &BC, Access Acc) {
...
bool InstDF::removeUse(const BinaryContext &BC, Access Acc,
                       Access Use) {
  unsigned AccIdx = calculateAccessIndex(BC, Acc);
...
const InstDF &getDF(const BinaryContext &BC, Access Acc) {
