...
struct Access {
  ...
  bool operator<(const Access &O) const {
    return std::tie(Inst, OpIdx, Space) < std::tie(O.Inst, O.OpIdx, Space);
  }
  bool operator==(const Access &O) const {
    return std::tie(Inst, OpIdx, Space) == std::tie(O.Inst, O.OpIdx, Space);
  }
  ...
  void next(const BinaryContext &BC) {
    assert(!isSentinel());
    const MCInstrDesc &II = BC.MII->get(Inst->getOpcode());
    ...
    case EXPLICIT:
      for (unsigned E = MCPlus::getNumPrimeOperands(*Inst); OpIdx != E; ++OpIdx)
        if (Inst->getOperand(OpIdx).isReg())
...
public:
  ...
  AccessList &getUses(Access &Def) { return DU.at(Def); }
  const AccessList &getUses(const Access &Def) const { return DU.at(Def); }

  bool isExternal(Access &Op) { return Op.Inst == &External; }
...
private:
  ...
  typedef enum { USE, DEF } AccessDir;

  typedef enum { EXPLICIT, IMPLICIT } AccessType;

  using RegToDefMap = SmallMapVector<MCPhysReg, Access, 8>;
  using BBToRegToDefMap = std::unordered_map<BinaryBasicBlock *, RegToDefMap>;

  template <AccessType Type, AccessDir Op>
  void addAccess(const BinaryFunction *BF, MCInst &Inst, MCPhysReg Reg,
                 unsigned Idx, RegToDefMap &LastDef);
